...
- Transaction lookup: `POST /getTransactionByHash`; if that does not find the tx after the scheduled tick, public verification falls back to `POST /getTransactionsForTick` with `tickNumber` and `filters.source`. `moneyFlew: true` means executed, `moneyFlew: false` means included but not executed.
